...
def main():
	...
	headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36'}
        r = requests.get(args.remote_URL, headers=headers)
        code = r.text
        ...
        if "etherscan.io" in args.remote_URL and not args.bytecode:
            try:
                begin = code.find('The Contract Address 0x')
		begin=begin+21
		end=begin+42
		filename=code[begin:end]
                filename += ".sol"
            except:
                pass
	    begin = code.find("/**")
	...
	with open(filename, 'w') as f:
            f.write(code)
	os.system("cp ./0x*.sol ./contracts/")
        #filename = os.path.join("contracts", filename)
	#f = open(filename, "w")
        #f.write(code)
	#f.close()
